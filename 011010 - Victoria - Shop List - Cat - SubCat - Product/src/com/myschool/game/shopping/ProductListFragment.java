...
public class ProductListFragment extends ListFragment {
	...
	void display(Bundle bundle) {
		...
		mCursor = databaseHelper.getProductCursor(mBundle.getInt("selected_shop_id"), mBundle.getString("selected_product_category"), mBundle.getString("selected_product_subcategory"));
		mAdapter = new SimpleCursorAdapter(getActivity(), 
				android.R.layout.simple_list_item_activated_1,
				mCursor, new String[] {"name"} , new int[] { android.R.id.text1 }, 0);
...
/*		setListAdapter(new ArrayAdapter<DummyContent.DummyItem>(getActivity(),
	...
	public void onListItemClick(ListView listView, View view, int position,
		...
		int productId = mCursor.getInt(mCursor.getColumnIndex(DatabaseHelper.KEY_ID));
		// Notify the active callbacks interface (the activity, if the
		// fragment is attached to one) that an item has been selected.
		mCallbacks.onItemSelected(productId);
	}
